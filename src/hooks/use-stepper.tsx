...
export function useStepper<const T extends readonly StepperItem<any, any>[]>(
  ...
  const [completedStepKeys, setCompletedStepKeys] = React.useState<Set<string>>(
    new Set(),
  );
  ...
  const nestedSchema = React.useMemo(() => createNestedSchema(steps), [steps]);
  const currentStepSchema = steps[activeStep]?.schema;
  const stepKeyToIndex = React.useMemo(() => createStepKeyMap(steps), [steps]);
    ...
    steps.forEach((step) => {
      ...
      if (!hasData) {
        ...
        try {
          ...
          const filledFields = Object.values(stepData as object).filter(
            (value) => value !== '' && value !== undefined && value !== null,
          ).length;
  ...
  const stepErrors = useMemo(() => {
    ...
    steps.forEach((step) => {
      const stepErrorObj = formErrors[step.key as keyof typeof formErrors];
  ...
  const validateStep = async (
    ...
    const stepFieldPaths = getStepFieldPaths<FormType>(step.key, step.schema);
  ...
  const validateAllSteps = async (): Promise<boolean> => {
    const allFieldPaths = steps
      ...
      .flatMap((step) =>
        step.schema ? getStepFieldPaths<FormType>(step.key, step.schema) : [],
      );
  ...
  const isCurrentStepValid = React.useMemo(() => {
    ...
    return stepFieldPaths.every((fieldPath) => {
      const fieldError = fieldPath
        .split('.')
        .reduce((errors, key) => errors?.[key], formState.errors as any);
      return !fieldError;
    });
