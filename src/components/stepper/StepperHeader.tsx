  ...
  const getTooltipContent = (step: any, index: number) => {
    ...
    return (
      ...
      >
        ...
        >
          {isCompleted && <div className="text-green-400">✓ Completed</div>}
        ...
        <div className="md:hidden">
          ...
          >
            {steps.map((step, index) => {
              const isActive = index === activeStep;
              const isCompleted = completedSteps.has(index);
              const isClickable = canGoToStep?.(index) && onStepClick;
                    ...
                    <div className="relative">
                      <motion.button
                        ...
                        className={cn(
                          ...
                          getStepStatusColor(step.key, index),
                      ...
                      >
                        <AnimatePresence mode="wait">
                          {getValidationIcon(step.key, index)}
                        </AnimatePresence>
                      </motion.button>
                      ...
                      <AnimatePresence>
                        {!isCompleted &&
                          stepValidationStatus[step.key] === 'invalid' && (
                            <motion.div
                              variants={validationDotVariants}
                              initial="initial"
                              animate={['animate', 'pulse']}
                              exit="exit"
                              className={cn(
                                'absolute -top-1 -right-1 rounded-full border-2 border-white bg-red-500',
                                minimal ? 'h-2 w-2' : 'h-3 w-3',
                              )}
                            />
                          )}
                        {!isCompleted &&
                          stepValidationStatus[step.key] === 'valid' && (
                            <motion.div
                              variants={validationDotVariants}
                              initial="initial"
                              animate="animate"
                        ...
                        <div className="relative">
                          <motion.button
                            ...
                            className={cn(
                              ...
                              minimal ? 'h-8 w-8' : 'h-12 w-12', // Smaller size in minimal mode
                              getStepStatusColor(step.key, index),
                              !isClickable && 'cursor-not-allowed',
                            )}
                            variants={stepCircleVariants}
                    ...
                    >
                      <motion.h3
                        className={cn(
                          ...
                          !isActive &&
                            !isCompleted &&
                            stepValidationStatus[step.key] === 'partial' &&
                            'text-yellow-600',
                          !isActive &&
                            !isCompleted &&
                            stepValidationStatus[step.key] === 'untouched' &&
                            'text-gray-500',
                        )}
                        animate={{
                    ...
                    >
                      <motion.div
                        className={cn(
                          ...
                          minimal ? 'w-full min-w-12' : 'w-full min-w-8',
                        )}
                        animate={{
