...
export default function Sidebar({ setNavUrl }: SidebarProps) {
  ...
  return (
    <div className="border-r border-subtle fixed">
      ...
      <aside
        className={cn(
          'bg-[#FFFFFF] font-Sora text-[#0F4539] h-[calc(100vh-3rem)] p-4 transition-all duration-300 z-10',
          collapsed ? 'w-16' : 'w-72',
          isOpen ? 'block' : 'hidden',
          collapsed ? 'fixed' : 'fixed lg:static top-12 left-0 lg:block',
        )}
      >
          ...
          {auth.isAuthenticated ? (
            <div className="relative">
                  ...
                  {!collapsed && (
                    <div className="flex flex-col items-start">
                      ...
                      <span className="text-sm font-semibold">
                        {auth.state.firstName} {auth.state.lastName}
                      </span>
                    </div>
                  )}
                </div>
              ...
              {showUserMenu && !collapsed && (
                <div className="absolute mt-1 left-0 bg-white border border-subtle rounded shadow w-full z-10">
